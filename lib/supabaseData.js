...
export async function loadUserState(userId) {
  ...
  const goals = goalsResult.data
    ? {
        ...
        workoutReminderTime: goalsResult.data.workout_goals?.workoutReminderTime || initial.goals.workoutReminderTime,
  ...
  const exercisesByWorkout = exerciseResult.data.reduce((acc, row) => {
    ...
    acc[row.workout_id].push({
      ...
      ...row.payload,
      heartRate: row.payload?.heartRate ?? row.heart_rate ?? ""
    });
    return acc;
  }, {});
  ...
  return {
    ...
    workoutAssistant: {
      ...
      workouts,
      templates: sanitizeWorkoutTemplates(templates)
    },
    remoteMeta: {
...
export async function saveUserState(userId, state) {
  ...
  const goalPayload = {
    ...
    workout_goals: {
      ...(state.workoutAssistant?.goals || defaultWorkoutAssistant.goals),
      workoutReminderTime: state.goals.workoutReminderTime || "18:00"
    },
    updated_at: now
  };
  ...
  await replaceRows("workout_exercises", userId, workouts.flatMap((workout) => {
    ...
    return (workout.exercises || []).map((exercise, index) => ({
      ...
      heart_rate: numberOrNull(exercise.heartRate),
  ...
  await replaceRows("workout_templates", userId, sanitizeWorkoutTemplates(state.workoutAssistant?.templates || []).map((template) => {
    const row = {
      user_id: userId,
      name: template.name,
      type: template.type,
    ...
    // Important: do not send id: null/undefined. Omit id so Postgres can use
    // the gen_random_uuid() default for imported or locally-created templates.
    if (isUuid(template.id)) row.id = template.id;
    return row;
  }));
...
export async function importLocalState(userId, localState) {
  await saveUserState(userId, sanitizeImportedState(localState));
}
...
function sanitizeImportedState(state) {
  return {
    ...state,
    workoutAssistant: {
      ...(state.workoutAssistant || defaultWorkoutAssistant),
      templates: sanitizeWorkoutTemplates(state.workoutAssistant?.templates || defaultWorkoutAssistant.templates)
    }
  };
}

function sanitizeWorkoutTemplates(templates = []) {
  return templates
    .filter(Boolean)
    .map((template, index) => {
      const cleaned = {
        ...template,
        name: template.name || `Workout Template ${index + 1}`,
        type: template.type || "Full body",
        intensity: template.intensity || "moderate",
        exercises: Array.isArray(template.exercises) ? template.exercises : []
      };
      if (!isUuid(cleaned.id)) delete cleaned.id;
      return cleaned;
    });
}
